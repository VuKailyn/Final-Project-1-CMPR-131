...
FitnessEvent::FitnessEvent(string* newNbhdList, int newSize, string fileName)// default constrctor
{
	...
	while (getline(partData, line))
	{
		...
		Participant* temp = new Participant(id, fName, lName, miles, size);
...
FitnessEvent::FitnessEvent(const FitnessEvent& otherEvent) // copy constructor
{
	...
	while (current)
	{
		...
		Participant* temp = new Participant(current->partID, current->fName, current->lName, newMiles, current->neighborhoodSize);
		temp->next = nullptr;
...
FitnessEvent& FitnessEvent::operator=(const FitnessEvent& RHS)// overloaded assig. oper.
{
	...
	while (otherCurrent)
	{
		...
		Participant* temp = new Participant(otherCurrent->partID, otherCurrent->fName, otherCurrent->lName, newMiles, otherCurrent->neighborhoodSize);
		if (listFirst == nullptr)
		{
			listFirst = listLast = temp;
		}
...
void FitnessEvent::displayParticipants() const
{
	...
	while (current != nullptr)
	{
		cout << current->fName << " " << current-> lName << endl;
		current = current->next;
	}
}
